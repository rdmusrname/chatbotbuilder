...
export default function NewsletterModal({ onClose }: NewsletterModalProps) {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>, enrollNewsletter: (props: DefaultFormFields) => void) {
    event.preventDefault();
    console.log({ email });
    if (email) {
      try {
        await fetch('https://autonoweb-newsletter.youssef02bigra.workers.dev/', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email, website: EnvVars.SITE_NAME}),
        });
        setEmail('');
      } catch (error) {
